...
export default function ReminderBanner({
  ...
  useEffect(() => {
    function update() {
      ...
      else if (mins < 1440) setTimeAgo(`${Math.floor(mins / 60)}h ${mins % 60}m ago`);
  ...
  // Send browser notification if tasks are incomplete
  useEffect(() => {
    if (incompleteCount > 0 && !notifSent) {
      // Wait 3 seconds then notify (after page has loaded)
      const timer = setTimeout(() => {
        sendNotification(
          "⏰ Tasks Waiting!",
          `You have ${incompleteCount} incomplete task${incompleteCount > 1 ? "s" : ""}. Don't break your streak!`
        );
      }, 3000);
      return () => clearTimeout(timer);
    }
  }, [incompleteCount, notifSent, sendNotification]);

  // Detect tab visibility change
  useEffect(() => {
    function handleVisibility() {
      if (document.visibilityState === "visible" && incompleteCount > 0) {
        new Notification("👋 Welcome Back!", {
          body: `You have ${incompleteCount} task${incompleteCount > 1 ? "s" : ""} waiting. Let's finish them!`,
          icon: "/favicon.ico",
        });
      }
    }
  ...
  return (
    <div
      className={`bg-gradient-to-r ${urgencyColors[urgency]} border rounded-2xl p-4 mb-6`}
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <p className="font-semibold">
            {incompleteCount} task{incompleteCount > 1 ? "s" : ""} incomplete
          </p>
          <p className="text-sm text-[var(--muted)] mt-0.5">
            {timeAgo !== "never"
              ? `Last checked in ${timeAgo} · ${urgencyMessages[urgency]}`
            ...
            {!("Notification" in window) ||
              (Notification.permission === "default" && (
                <button
                  ...
                  className="text-xs px-3 py-1.5 rounded-lg border border-[var(--border)] text-[var(--muted)] hover:bg-[var(--card)] transition-colors"
                >
                  Enable Reminders
                </button>
              ))}
